...
async function generateDocumentPDF(invoice, items, res, disposition = 'attachment') {
  ...
  const asBuffer = disposition === 'buffer' || !res;
  let chunks = [];
  if (asBuffer) {
  ...
  if (isDunning) {
    ...
    introText = 'Sehr geehrte Damen und Herren,\ndiese Rechnung wurde storniert. Die nachfolgenden Positionen werden gutgeschrieben bzw. aufgehoben:';
  } else {
  ...
  if (isDunning) {
    ...
    const skontoP = parseFloat(invoice.skonto_percent != null ? invoice.skonto_percent : (firma.skonto_percent || 0)) || 0;
    const skontoD = parseInt(invoice.skonto_days != null ? invoice.skonto_days : (firma.skonto_days || 0), 10) || 0;
    ...
    if (skontoP > 0 && skontoD > 0) {
      const skontoAmt = total * (skontoP / 100);
      paymentNote += `\nBei Zahlung innerhalb von ${skontoD} Tagen gewähren wir ${skontoP.toLocaleString('de-DE')} % Skonto (${fmt(skontoAmt)}).`;
    }
    if (firma.zahlungsbedingungen) {
      paymentNote += '\n' + firma.zahlungsbedingungen;
    }
  }
...
async function generateDocumentPDFBuffer(invoice, items) {
  return generateDocumentPDF(invoice, items, null, 'buffer');
}

module.exports = { generateDocumentPDF, generateDocumentPDFBuffer };
