const db = require('./config/database'); // bzw. je nachdem, wie du dbQuery importierst
...
async function seedArticles() {
  try {
    ...
    for (const article of initialArticles) {
      // Prüfen, ob der Artikel bereits existiert (über die globale dbQuery Funktion)
      // Falls dbQuery global in deiner app läuft, ansonsten entsprechend importieren:
      let existing;
      try {
        existing = typeof dbQuery === 'function' 
          ? await dbQuery('SELECT id FROM articles WHERE title = ?', [article.title])
          : null;
      } catch (e) {
        // Fallwiderspruch bei Platzhaltern abfangen
      }

      // Wenn noch nicht vorhanden, sicher einfügen
    ...
    console.log('Artikelstamm erfolgreich befüllt!');
  } catch (err) {
