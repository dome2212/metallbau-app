...
// OpenRouter-Fallback für Vision (mehrere Bilder)
...
async function callOpenRouterVisionMulti(prompt, imageBuffers) {
...
async function callAIWithImages(prompt, imageBuffers) {
  ...
  if (process.env.GEMINI_API_KEY) {
    try { return await callGeminiVisionMulti(prompt, imageBuffers); } catch (err) { lastError = err; }
  }
  // Gemini fehlgeschlagen → OpenRouter als letzter Fallback
  if (process.env.OPENROUTER_API_KEY) {
    return await callOpenRouterVisionMulti(prompt, imageBuffers);
  }
    ...
    const [filesRes, appRes, photosRes, measurementsRes, notesRes, tasksRes, usersRes, hoursRes, statusLogRes, lagerRes, entnahmenRes] = await Promise.all([
      ...
      dbQuery(
        `SELECT le.id, le.lager_item_id, le.project_id, le.user_id,
                ...
                li.bezeichnung, li.profil
         FROM lager_entnahmen le
         JOIN lager_items li ON le.lager_item_id = li.id
         WHERE le.project_id = ?
...
router.post('/:id/material/add', async (req, res) => {
  ...
  try {
    // Lagerbestand + Einheitspreis holen
    const item = await dbQuery('SELECT menge, einheit, einheitspreis FROM lager_items WHERE id = ?', [lager_item_id]);
    if (!item.rows[0]) return res.redirect(`/projects/${projectId}`);
    const neuerBestand = parseFloat(item.rows[0].menge) - mengeNum;
    const ep = parseFloat(item.rows[0].einheitspreis || 0);
    const gp = Math.round(ep * mengeNum * 100) / 100;
    await dbQuery(
      'INSERT INTO lager_entnahmen (lager_item_id, project_id, user_id, menge, einheit, notiz, einheitspreis, gesamtpreis) VALUES (?, ?, ?, ?, ?, ?, ?, ?)',
...
router.post('/:id/ai-summary', async (req, res) => {
  ...
  try {
    ...
    const text = stripThinking(await callAI(prompt, 600));
...
router.post('/:id/ai-checklist', async (req, res) => {
  ...
  try {
    ...
    const text  = stripThinking(await callAI(prompt, 400));
...
router.post('/:id/generate-quote', async (req, res) => {
  ...
  try {
    ...
    const text = stripThinking(await callAI(prompt, 800));
    res.json({ quote: text });
  } catch (err) {
...
router.post('/:id/generate-quote-with-images', imageUpload.array('images', 3), async (req, res) => {
  ...
  try {
    ...
    const text = stripThinking(imageBuffers.length > 0
      ? await callAIWithImages(prompt, imageBuffers)
      : await callAI(prompt, 800));
