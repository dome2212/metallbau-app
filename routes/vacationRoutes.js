...
router.post('/status', requireAdmin, async (req, res) => {
  ...
  try {
    ...
    const vacRes = await dbQuery(
      `SELECT v.user_id, v.type, v.start_date, v.end_date, u.whatsapp_phone, u.whatsapp_api_key, u.whatsapp_notify
    ...
    if (vac) {
      const emoji = status === 'Genehmigt' ? '✅' : '❌';
      sendPush({
        title: `${emoji} ${vac.type}-Antrag ${status}`,
        body:  `Dein Antrag (${vac.start_date} bis ${vac.end_date}) wurde ${status}.`,
        url:   '/vacations'
      }, vac.user_id).catch(() => {});
    }
