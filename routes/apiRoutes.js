  ...
  if (process.env.GEMINI_API_KEY) {
    for (const gModel of ['gemini-2.0-flash-lite', 'gemini-2.0-flash']) {
      try {
        ...
        if (!gr.ok) { lastError = gdata; continue; }
        return res.json({ reply: gdata?.candidates?.[0]?.content?.parts?.[0]?.text || '' });
      } catch (err) { lastError = err; }
    }
  }
  // 3) OpenRouter als letzter Fallback (alle kostenlosen Vision-Modelle)
  if (process.env.OPENROUTER_API_KEY) {
    for (const orModel of [
      ...
      try {
        ...
        const ordata = await or.json();
        if (!or.ok) { lastError = ordata; continue; }
        return res.json({ reply: ordata?.choices?.[0]?.message?.content || '' });
      } catch (err) { lastError = err; }
    }
  }
  res.status(500).json({ error: 'KI-Bildanalyse fehlgeschlagen.', detail: lastError });
});
